...
**First-Run Setup**:
The initialization process for a brand new Vroom HR deployment. A web-based wizard where the admin configures the Organization, Google OAuth, and allowed domains. It runs exactly once and permanently locks upon completion.
_Avoid_: Install script, onboarding (onboarding is for employees).

**Setup Token**:
A one-time, random alphanumeric string generated by the backend and printed to the server console during First-Run Setup. Used to verify the identity of the administrator before Google OAuth is configured.
_Avoid_: License key, Admin password, Secret key.
