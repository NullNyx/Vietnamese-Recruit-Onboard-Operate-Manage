...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "credential_format_version", sa.Integer(), nullable=False, server_default=sa.text("1")
        ),
        sa.Column(
            "credential_key_version", sa.Integer(), nullable=False, server_default=sa.text("1")
        ),
        ...
        sa.UniqueConstraint(
            "organization_singleton_key", name="uq_organization_google_connections_singleton_key"
        ),
    )
