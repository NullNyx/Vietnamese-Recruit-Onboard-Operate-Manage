...
def upgrade() -> None:
    ...
    if 'corrected_by_user_id' not in columns:
        op.add_column('attendance_records', sa.Column('corrected_by_user_id', sa.Uuid(), nullable=True))
    if 'corrected_at' not in columns:
        op.add_column('attendance_records', sa.Column('corrected_at', sa.DateTime(timezone=True), nullable=True))
    if 'correction_reason' not in columns:
        op.add_column('attendance_records', sa.Column('correction_reason', sa.Text(), nullable=True))
    if 'previous_check_in_at' not in columns:
        op.add_column('attendance_records', sa.Column('previous_check_in_at', sa.DateTime(timezone=True), nullable=True))
    if 'previous_check_out_at' not in columns:
        op.add_column('attendance_records', sa.Column('previous_check_out_at', sa.DateTime(timezone=True), nullable=True))
...
def downgrade() -> None:
    ...
    if 'fk_attendance_records_corrected_by_user_id' in fk_names:
        op.drop_constraint('fk_attendance_records_corrected_by_user_id', 'attendance_records', type_='foreignkey')
