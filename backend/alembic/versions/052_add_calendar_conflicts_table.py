...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "resolved_at", sa.DateTime(timezone=True), nullable=True, default=None
        ),
        sa.Column(
            "created_at",
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_calendar_conflicts_interview_id", table_name="calendar_conflicts"
    )
    op.drop_index(
        "ix_calendar_conflicts_candidate_id", table_name="calendar_conflicts"
    )
