...
Truncates core tables with CASCADE so all dependent rows are cleaned.
Optionally inserts a minimal admin user and organization_settings row
(``--setup-complete``) for tests that need a pre-configured deployment
without running the First-Run Setup wizard.
...
def _seed_minimal_data(session) -> None:
    """Insert minimal admin user + organization_settings row."""
