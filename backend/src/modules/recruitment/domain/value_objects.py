...
class FieldProvenance(BaseModel):
    """Evidence and review state for one value in a structured CV draft."""

    source_fragment: str = Field(default="", max_length=1000)
    status: str = Field(default="missing", max_length=30)
    needs_confirmation: bool = True


class CVFieldValidation(BaseModel):
    """A field-level accuracy or completeness finding."""
...
class ParsedCV(BaseModel):
    """Structured CV draft extracted by AI, never a Candidate write.

    The optional provenance and validation metadata is deliberately part of
    the draft contract so callers cannot mistake extracted values for facts.
    """
    ...
    validation_errors: list[CVFieldValidation] = Field(default_factory=list)
