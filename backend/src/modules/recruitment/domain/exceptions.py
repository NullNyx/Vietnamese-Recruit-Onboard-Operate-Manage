...
class JobOpeningNotOpenError(RecruitmentError):
    ...
    Raised when trying to assign a Candidate to a Job Opening that is
    not in the 'open' status, since only open Job Openings accept new
    Candidate assignments per ADR-0014.
    """

    status_code = 422
    error_code = "JOB_OPENING_NOT_OPEN"
    message = "Job Opening is not open for candidate assignment"


class CandidateTerminalStatusError(RecruitmentError):
    """Candidate is in a terminal status and cannot be assigned.

    Raised when trying to assign, reassign, or unassign a Candidate
    whose status is accepted, rejected, or archived — assignment
    changes are only allowed while the Candidate is in new, reviewing,
    or interview_scheduled status.
    """

    status_code = 409
    error_code = "CANDIDATE_TERMINAL_STATUS"
    message = "Cannot change assignment for a candidate in terminal status"

    def __init__(self, current_status: str) -> None:
        """Initialize CandidateTerminalStatusError.

        Args:
            current_status: The Candidate's current terminal status value.
        """
        self.current_status = current_status
        self.message = (
            f"Cannot change assignment for candidate with status '{current_status}'; "
            f"only new, reviewing, or interview_scheduled candidates can be reassigned"
        )
        super().__init__(self.message)


class CandidateNotAssignedError(RecruitmentError):
    """Candidate is not assigned to any Job Opening.

    Raised when trying to unassign a Candidate that has no
    job_opening_id.
    """

    status_code = 409
    error_code = "CANDIDATE_NOT_ASSIGNED"
    message = "Candidate is not assigned to any Job Opening"
