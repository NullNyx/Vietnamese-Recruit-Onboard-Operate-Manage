...
class CryptoUtils:
    ...
    def __init__(self, encryption_key_b64: str, *, key_version: int = 1, previous_encryption_key_b64: str | None = None) -> None:
        ...
        if previous_encryption_key_b64:
            ...
            if len(previous) != 32:
                raise ValueError(f"Encryption key must be exactly 32 bytes, got {len(previous)} bytes")
    ...
    def encrypt_versioned(self, plaintext: str) -> VersionedSecret:
        ...
        return VersionedSecret(self._FORMAT_VERSION, self._key_version, base64.b64encode(nonce).decode(), base64.b64encode(encrypted).decode())

    def encrypt(self, plaintext: str) -> str:
        s = self.encrypt_versioned(plaintext)
        payload = repr({"format_version": s.format_version, "key_version": s.key_version, "nonce": s.nonce, "ciphertext": s.ciphertext})
    ...
    def decrypt(self, ciphertext: str) -> str:
        ...
        if raw.startswith(b"{"):
            try:
                ...
                return self._decrypt_with_available_keys(base64.b64decode(p["nonce"]), base64.b64decode(p["ciphertext"]))
