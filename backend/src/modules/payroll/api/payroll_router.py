...
def create_period(
    ...
    period = service.create_period(data.month, data.year)
    return period
...
def calculate_payroll(
    ...
    try:
        payslips = service.calculate_all_employees(period_id)
        return payslips
    except Exception as e:
...
def confirm_period(
    ...
    try:
        period = service.confirm_period(period_id, confirmed_by)
        return period
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
def mark_period_paid(
    ...
    try:
        period = service.mark_period_paid(period_id)
        return period
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
def get_employee_payslips(
    ...
    payslips = service.get_employee_payslips(employee_id)
    return payslips
