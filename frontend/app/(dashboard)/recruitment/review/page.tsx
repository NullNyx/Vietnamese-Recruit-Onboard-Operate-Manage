  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-2 text-indigo-600">
        ...
        <h1 className="text-xl font-bold text-slate-900">Review CV (Parse Queue)</h1>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="flex items-center justify-between text-xs text-slate-500">
            <span>Trang {page} · {data?.total ?? 0} bản ghi</span>
            <div className="flex gap-2">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1} className="px-2 py-1 bg-white border border-slate-200 rounded disabled:opacity-40">Trước</button>
              <button onClick={() => setPage((p) => p + 1)} disabled={items.length < 12} className="px-2 py-1 bg-white border border-slate-200 rounded disabled:opacity-40">Sau</button>
            </div>
          </div>
        </>
      )}
    ...
    <div className="p-4 bg-white rounded-2xl border border-slate-200 shadow-sm shadow-slate-100">
      ...
      {item.processing_error && (
        <div className="p-2.5 bg-rose-50 border border-rose-200 rounded-lg text-xs text-rose-600 flex items-start gap-2 mb-2">
          ...
          <span>Lỗi parse: {item.processing_error}</span>
        </div>
      )}
      ...
      {!editing && item.parsed_cv_data && (
        <div className="text-xs space-y-1 text-slate-600">
          {item.parsed_cv_data.name && <p><span className="font-mono text-slate-400">tên:</span> {item.parsed_cv_data.name}</p>}
          {item.parsed_cv_data.email && <p><span className="font-mono text-slate-400">email:</span> {item.parsed_cv_data.email}</p>}
      ...
      {!item.parsed_cv_data && !editing && <p className="text-xs text-slate-400">Chưa có dữ liệu parse.</p>}

      {/* Correction form */}
      {editing && (
        <div className="mt-2 p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
          <p className="text-[10px] font-mono uppercase text-slate-500">Correction (HR xác nhận, phục vụ evaluation set — không online learning)</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Tên" className="px-2.5 py-1.5 text-xs bg-white border border-slate-200 rounded-lg" />
            <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" className="px-2.5 py-1.5 text-xs bg-white border border-slate-200 rounded-lg px-2.5" />
            <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="SĐT" className="px-2.5 py-1.5 text-xs bg-white border border-slate-200 rounded-lg" />
            <input value={skills} onChange={(e) => setSkills(e.target.value)} placeholder="Skills (phẩy)" className="px-2.5 py-1.5 text-xs bg-white border border-slate-200 rounded-lg" />
          </div>
          <textarea value={summary} onChange={(e) => setSummary(e.target.value)} placeholder="Tóm tắt" rows={2} className="w-full px-2.5 py-1.5 text-xs bg-white border border-slate-200 rounded-lg" />
          <div className="flex justify-end gap-2">
            <button onClick={onToggleEdit} className="px-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg">Hủy</button>
            <button onClick={submit} disabled={pending} className="px-3 py-1.5 text-xs bg-emerald-600 text-white rounded-lg disabled:opacity-50 flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5" /> Nộp correction</button>
          </div>
        </div>
      )}
      ...
      {!editing && (
        <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-slate-100">
          <button onClick={onToggleEdit} className="text-[11px] px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 flex items-center gap-1"><Pencil className="w-3 h-3" /> Correction</button>
          {(item.processing_status === 'failed' || item.processing_status === 'permanently_failed' || item.processing_status === 'needs_review') && (
            <button onClick={onRetry} disabled={pending} className="text-[11px] px-2.5 py-1 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 flex items-center gap-1 disabled:opacity-50"><RefreshCw className="w-3 h-3" /> Retry parse</button>
          )}
          <button onClick={onDismiss} disabled={pending} className="text-[11px] px-2.5 py-1 bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-100 flex items-center gap-1 disabled:opacity-50"><Ban className="w-3 h-3" /> Dismiss</button>
        </div>
      )}
    </div>
  );
