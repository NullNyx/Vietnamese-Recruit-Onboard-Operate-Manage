  ...
  return (
    <div className="lg:col-span-1 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-3 py-2 border-b border-slate-100 flex items-center gap-2">
        ...
        <button onClick={onRefresh} className="ml-auto p-1 rounded hover:bg-slate-100 text-slate-400">
      ...
      <div className="max-h-[60vh] overflow-y-auto">
        {isLoading ? (
          ...
          <div className="p-4 text-xs text-rose-500">Lỗi: {apiErrorText(error)}</div>
        ) : (messages?.length ?? 0) === 0 ? (
          <EmptyState
