...
business modules — recruitment (candidates, CVs, interviews) and onboarding
(progress). It must decide where to live in the backend module layout.
...
services of other modules (recruitment, onboarding); it holds no business logic
of its own.
...
1. **Put it inside `recruitment/`.** Rejected: the assistant also reads
   onboarding data, which would force recruitment to depend on onboarding, and it
